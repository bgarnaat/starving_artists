...
function checkEvents(input) {
  scheduled_events.forEach(function(item, index, array) {
    var check_name = input.in_show !== "true" ? input.in_show === item.show.show_name : true;
    var check_date = input.in_date !== true ? (input.in_date >= item.date_start || input.in_date <= item.date_end) : true;
    var check_venue = input.in_venue !== true ? input.in_venue === item.venue.venue_name : true;
